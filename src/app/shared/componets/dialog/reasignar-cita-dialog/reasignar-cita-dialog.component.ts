...
export class ReasignarCitaDialogComponent implements OnInit {
  ...
  applyFilter(event: Event) {
    ...
    if (this.citas.paginator) {
      this.citas.paginator.firstPage();
    }
  }
  ...
  loadCitas() {
    this.citasService.listarCitas().subscribe({
      next: (response: any) => {
        ...
        const citasFiltradas = response.citas.filter(
          (cita: any) => cita.estado_ingreso === 'N'
        );
  ...
  reasignarCitas() {
    if (this.reasignarForm.valid) {
      ...
      if (
        moment(fechaSeleccionada).isBefore(this.minDate) ||
        moment(fechaSeleccionada).isAfter(this.maxDate)
      ) {
        alert(
          'Por favor selecciona una fecha válida dentro del rango permitido.'
        );
        return;
      }

      const formattedDate = this.datePipe.transform(
        this.reasignarForm.value.fecha,
        'dd/MM/yyyy'
      );
